...
export default function App() {
  ...
  const phases = useMemo(() => ['Todos', ...new Set(games.map((game) => game.phase))], []);

  const filteredGames = useMemo(() => {
    const term = search.trim().toLowerCase();
    return games.filter((game) => {
      const matchPhase = phaseFilter === 'Todos' || game.phase === phaseFilter;
      const matchSearch = !term || [game.home, game.away, game.group, game.stadium, game.phase]
        .join(' ')
        .toLowerCase()
        .includes(term);
  ...
  const sections = useMemo(() => groupBySection(filteredGames), [filteredGames]);

  const completed = useMemo(() => {
    return games.filter((game) => predictions[game.id]?.home !== '' && predictions[game.id]?.away !== '').length;
  }, [predictions]);
  ...
  function exportPdf() {
    ...
    sectionEntries.forEach(([sectionName, sectionGames], sectionIndex) => {
      ...
      doc.text(`${completed}/${games.length} jogos preenchidos`, margin, 34);
      ...
      sectionGames.forEach((game) => {
        ...
        doc.text(location, margin + 5, y + 23, { maxWidth: pageWidth - margin * 2 - 10 });
      ...
      doc.setFontSize(7);
      doc.setTextColor(...colors.muted);
      doc.text(`Página ${sectionIndex + 1} de ${sectionEntries.length}`, pageWidth / 2, pageHeight - 8, { align: 'center' });
    });
    ...
    <main className="page">
      <header className="hero">
        <div className="hero-copy">
          ...
          <p>Preencha seus palpites jogo a jogo e exporte um PDF organizado por grupos e fases.</p>
        </div>
      ...
      <section className="intro-card">
        <div>
          <span className="mini-label">Exportação</span>
          <h2>PDF dividido por Grupo A, Grupo B, Grupo C e demais fases</h2>
        </div>
        <p>O arquivo sai paginado em A4, com uma página para cada grupo da fase inicial e páginas separadas para o mata mata.</p>
      </section>
                  ...
                  <article className="game-card" key={game.id}>
                    ...
                    <div className="teams">
                      ...
                      <input
                        ...
                        onChange={(event) => updatePrediction(game.id, 'home', event.target.value)}
                        inputMode="numeric"
                        placeholder="0"
                      />
                      <strong>x</strong>
                      <input
                        ...
                        onChange={(event) => updatePrediction(game.id, 'away', event.target.value)}
                        inputMode="numeric"
                        placeholder="0"
                      />
